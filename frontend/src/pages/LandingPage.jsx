...
function FeatureCard({ icon, title, desc }) {
  return (
    ...
    onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--violet-border)'; e.currentTarget.style.background = 'var(--ink-3)'; e.currentTarget.style.boxShadow = '0 0 24px var(--violet-glow)' }}
    onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'var(--ink-2)'; e.currentTarget.style.boxShadow = 'none' }}
    >
...
export default function LandingPage({ onEnter }) {
  return (
    // KEY FIX: overflowY: 'auto' + height: '100%' makes it scrollable
    <div style={{ background: 'var(--ink)', color: 'var(--text)', overflowY: 'auto', height: '100%' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 26, height: 26, background: 'var(--violet)', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 14px var(--violet-glow)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <a href="#how" style={{ fontSize: 13, color: 'var(--text-2)', textDecoration: 'none' }}>How it works</a>
          <button onClick={onEnter} style={{
            height: 34, padding: '0 18px', borderRadius: 'var(--radius-md)',
            border: 'none', background: 'var(--violet)', color: '#fff',
            fontSize: 13, fontWeight: 500, cursor: 'pointer',
            fontFamily: 'var(--font-sans)', boxShadow: '0 0 16px var(--violet-glow)',
      ...
      <section style={{
        ...
        <div className="stagger" style={{ maxWidth: 700, position: 'relative' }}>
          ...
          <p style={{
            ...
            Jump to the <em style={{ color: 'var(--text)', fontStyle: 'normal', fontWeight: 500 }}>exact moment</em> the answer comes from.
          </p>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={onEnter} style={{
              height: 50, padding: '0 30px', borderRadius: 'var(--radius-lg)',
              border: 'none', background: 'var(--violet)', color: '#fff',
              fontSize: 15, fontWeight: 600, cursor: 'pointer',
              fontFamily: 'var(--font-sans)', boxShadow: '0 0 32px var(--violet-glow)',
              transition: 'all var(--transition)'
            }}
            ...
            >Open workspace →</button>
            <a href="#how" style={{
              height: 50, padding: '0 24px', borderRadius: 'var(--radius-lg)',
        ...
        <div className="hero-float" style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr 230px', height: 260 }}>
            <div style={{ borderRight: '1px solid var(--border)', padding: 10, display: 'flex', flexDirection: 'column', gap: 5 }}>
              {[1,2,3].map(i => (
                <div key={i} style={{ height: i === 1 ? 44 : 36, borderRadius: 7, background: i === 1 ? 'var(--violet-dim)' : 'var(--ink-3)', border: `1px solid ${i === 1 ? 'var(--violet-border)' : 'transparent'}` }} />
              ))}
              <div style={{ height: 1, background: 'var(--border)', margin: '3px 0' }} />
              {[1,2,3].map(i => <div key={i} style={{ height: 28, borderRadius: 6, background: 'var(--ink-3)' }} />)}
            </div>
            <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 9 }}>
              <div style={{ height: 90, borderRadius: 10, background: 'var(--violet-dim)', border: '1px solid var(--violet-border)' }} />
              {[1,2,3].map(i => <div key={i} className="skeleton" style={{ height: 44, borderRadius: 8 }} />)}
            </div>
            <div style={{ borderLeft: '1px solid var(--border)', padding: 10, display: 'flex', flexDirection: 'column', gap: 7 }}>
              <div style={{ height: 44, borderRadius: 10, background: 'var(--violet-dim)', border: '1px solid var(--violet-border)', marginLeft: '15%', width: '85%' }} />
              <div style={{ height: 80, borderRadius: 8, background: 'var(--ink-3)' }} />
              <div style={{ height: 24, borderRadius: 99, background: 'var(--teal-dim)', border: '1px solid var(--teal-border)', width: 90 }} />
              <div style={{ flex: 1 }} />
      ...
      {/* use cases — new section replacing "The Problem" */}
      <section style={{ padding: '80px 24px', maxWidth: 860, margin: '0 auto' }}>
        <p style={{ fontSize: 12, color: 'var(--violet-2)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14, textAlign: 'center' }}>Works for everything</p>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(26px,4vw,40px)', fontWeight: 700, lineHeight: 1.2, letterSpacing: '-0.02em', marginBottom: 36, textAlign: 'center' }}>
          Not just tutorials
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10 }}>
          {[
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--violet-border)'; e.currentTarget.style.background = 'var(--ink-3)' }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'var(--ink-2)' }}
            >
      ...
      <section id="how" style={{ padding: '80px 24px', maxWidth: 560, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          <StepCard num="1" title="Paste any YouTube URL" desc="We fetch the full transcript, chunk it semantically, and embed it into a vector database. Takes about 10 seconds." />
          <StepCard num="2" title="Ask anything" desc="Every answer is retrieved from actual transcript chunks — not hallucinated. We cite the exact second." />
          <StepCard num="3" title="Jump to the exact moment" desc="Click any source chip. Opens YouTube at precisely that timestamp. Every answer is verifiable." />
        </div>
      </section>
      ...
      <section id="features" style={{ padding: '80px 24px', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 14 }}>
          ...
          <FeatureCard icon="🔍" title="Semantic Search" desc="Type a concept. Find every moment it's discussed, ranked by relevance. Exact keyword matches always surface first." />
          <FeatureCard icon="📚" title="Compare Videos" desc="Load up to 3 videos. Ask which covers the topic better. Get a grounded comparison, not a guess." />
          ...
          <FeatureCard icon="💬" title="Conversation Memory" desc="Follow-up questions actually work. 'Why is that?' understands context. Full history retained per session." />
          <FeatureCard icon="📌" title="Chat History" desc="Every session saved with the videos used. Grouped by date. Nothing gets lost between visits." />
        </div>
      </section>

      {/* CTA */}
      <section style={{
        margin: '0 24px 80px', maxWidth: 860,
        marginLeft: 'auto', marginRight: 'auto',
        padding: '60px 40px', borderRadius: 'var(--radius-2xl)',
        background: 'var(--ink-2)', border: '1px solid var(--violet-border)',
        textAlign: 'center', boxShadow: '0 0 60px var(--violet-glow)'
      }}>
        ...
        <button onClick={onEnter} style={{
          height: 50, padding: '0 32px', borderRadius: 'var(--radius-lg)',
          border: 'none', background: 'var(--violet)', color: '#fff',
          fontSize: 15, fontWeight: 600, cursor: 'pointer',
          fontFamily: 'var(--font-sans)', boxShadow: '0 0 32px var(--violet-glow)',
          transition: 'all var(--transition)'
      ...
      <footer style={{ borderTop: '1px solid var(--border)', padding: '24px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, color: 'var(--text-3)' }}>· No account needed · No hallucination · Your data stays local</span>
        </div>
        <span style={{ fontSize: 12, color: 'var(--text-3)' }}>Built with FastAPI · ChromaDB · Groq · sentence-transformers</span>
      </footer>
    </div>
  )
}